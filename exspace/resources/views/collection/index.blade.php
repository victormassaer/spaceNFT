...
@section('content')
    ...
    <section class="mb-8">
        ...
        <select class="m-6 py-2 px-4 pr-8 rounded border" name="category" id="category">
            @foreach($collections as $collection)
                <option value="{{ $collection->category }}">{{ $collection->category }}</option>
            @endforeach
        </select>
        <button class="duration-700 transform bg-indigo-500 hover:bg-blue-400 text-gray-100 font-bold px-6 py-2 rounded"><span>Filter</span></button>
            ...
            @foreach($collections as $collection)
                <div class="w-full shadow-xl rounded-lg p-12 flex flex-col justify-center items-center">
                    ...
                    <div class="text-center mb-2">
                        ...
                        <span class="text-sm">Category: {{ $collection->category }}</span>
                    </div>
